...
GPIO.setup(SW2, GPIO.IN, pull_up_down = GPIO.PUD_DOWN) # 내부풀다운 사용
GPIO.setup(SW3, GPIO.IN, pull_up_down = GPIO.PUD_DOWN) # 내부풀다운 사용
GPIO.setup(SW4, GPIO.IN, pull_up_down = GPIO.PUD_DOWN) # 내부풀다운 사용
...
for i in range(10):  # 10회 반복 
    ...
    if judgment == 0:     # 틀린 버튼을 누르는 경우
        print("bad")
    elif judgment2 == 1:  # 버튼을 시간내에 누르지 못한 경우
        print("miss")
