...
led_pin_dict = {'red': 4, 'green': 5, 'blue': 6}     # LED 핀들을 딕셔너리 변수로 선언
...
def home():                                     # 여기서 led1sec.html을 화면에 보여줌
    return render_template('KYJmission05.html') 
...
def whole_control(state):                     # 모든 LED를 한번에 제어하기 위한 뷰함수
    if state == 0 or state == 1:                            # 입력받은 state가 0이나 1(ON, OFF 버튼을 눌렀으면)
    ...
    elif state == 2:                                        # 입력받은 state가 2이면(blink 버튼을 눌렀으면)
        for color_idx in ['red', 'green', 'blue']:          # 각각의 색상값에 대해
